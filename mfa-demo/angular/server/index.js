// PingOne Native Flows MFA Demo — Angular backend (Node.js / Express)
//
// This server exposes two JSON API endpoints consumed by the Angular frontend:
...
//     the Angular frontend never touches PingOne cookies directly.
...
// cookies are never exposed to the Angular client.
...
// Angular CLI builds to client/dist/<project>/browser — point this at that
// folder after running `ng build` in the client directory.
// In development the Angular CLI dev server (port 4200) uses proxy.conf.json
// to forward /api/* requests to this Express server on port 3000.
const angularDist = path.join(__dirname, '..', 'client', 'dist', 'p1-mfa-demo-angular', 'browser');
